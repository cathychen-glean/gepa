...
GLEAN_CHAT_EVAL_SET_NAME = "Glean Chat V2 Medium"
SCIO_PROD_DEPLOYMENT_IDS = ["scio-prod"]
...
def _make_evalset(versions: list[str], *, deployment_ids: list[str] | None = None) -> list[ALDataInst]:
...
def _resolve_eval_version_split(args: argparse.Namespace, evalcli: EvalCliClient) -> tuple[list[str], list[str]]:
    ...
    if args.train_eval_versions:
        ...
        rows = evalcli.list_eval_set_versions(
            eval_set_name=GLEAN_CHAT_EVAL_SET_NAME, deployment_ids=list(SCIO_PROD_DEPLOYMENT_IDS)
        )
