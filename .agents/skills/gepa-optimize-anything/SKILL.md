...
# that provider's key (OPENAI_API_KEY), or pass your own id (e.g. "anthropic/Codex-sonnet-4-6" with
...
# Agentic backends (autoresearch, meta_harness) additionally need the `Codex` CLI on PATH (plus
# `jq` for the generated eval.sh):
npm install -g @anthropic-ai/Codex   # or: curl -fsSL https://Codex.ai/install.sh | bash
#   ...then run `Codex` once to authenticate.
    ...
    config=OptimizeAnythingConfig(
        ...
        engine_config={              # gepa backend: a GEPAConfig-shaped dict, validated strictly —
            "reflection": {          #   an unknown key raises TypeError immediately (fail fast)
                ...
                "reflection_lm": "anthropic/Codex-sonnet-4-6",
...
- **Agentic backends (`autoresearch`, `meta_harness`) shell out to the `Codex` CLI** and abort at
...
- `scripts/preflight.py` — validate creds / proposer LM / `Codex` CLI before launching.
